...
public class Rose extends WaterActor {
    ...
    public void executeUpdate(World world, ImageLibrary imageLibrary, EventScheduler eventScheduler) {
        setImageIndex(getImages().size() * getWater() / Rose.ROSE_WATER_LIMIT);
        if (getWater() >= Rose.ROSE_WATER_LIMIT) {
            Actor nymph = new Nymph(getPosition(), imageLibrary.get(Nymph.NYMPH_KEY), Rose.ROSE_TO_NYMPH_UPDATE_PERIOD);
